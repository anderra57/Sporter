<?php
    ...
    function tokens_grupo($usuario) {
        global $conn;
        $sql = "SELECT id from common where id_team = (select t.id from common as c join users as u join teams as t on c.id_team = t.id and u.id = c.id where u.username = '$usuario');";
        $result = $conn->query($sql);
        $tokens_array = [];
